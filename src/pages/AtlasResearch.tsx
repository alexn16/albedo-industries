...
export default function AtlasResearch() {
  ...
  const active=atlasResearchRecords.filter(record=>!closed.includes(record)&&/active|continue|validation/i.test(record.status))
  const universe=atlasResearchRecords.filter(record=>!closed.includes(record)&&!active.includes(record))
  useAtlasMetadata({
...
function ResearchGroup({title,note,records}:{title:string;note:string;records:typeof atlasResearchRecords}){if(!records.length)return null;return <section className="pt-12"><h3 className="text-2xl font-semibold">{title}</h3><p className="mt-2 max-w-2xl text-sm text-zinc-500">{note}</p><div className="mt-5 divide-y divide-zinc-200 border-y border-zinc-200">{records.map(record => <article className="grid gap-4 py-7 md:grid-cols-[8rem_1fr_12rem]" key={record.id}>
