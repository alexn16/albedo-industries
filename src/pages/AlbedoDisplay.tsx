...
function PilotForm() {
  ...
  return <form className="ad-pilotform" onSubmit={submit} noValidate>
    ...
    <div className="ad-formfooter"><p>By applying, you agree that Albedo Industries may use these details to respond to your enquiry. Read our <Link to="/privacy">privacy notice</Link>.</p><button className="ad-button ad-button--primary" disabled={state === 'sending'}>{state === 'sending' ? 'Sending…' : 'Apply for a pilot'}</button></div>
  </form>
}
...
export default function AlbedoDisplay() {
  ...
  return <article className="ad-page ad-landing">
    <ProductNavigation />
    <header className="ad-hero"><div className="ad-wrap ad-hero__grid"><div className="ad-hero__copy"><span className="ad-status">Concept · component validation</span><p className="ad-kicker">PHONE-POWERED · PERSISTENT · FOCUSED</p><h1>A simple display for information that should stay visible.</h1><p className="ad-lede">Albedo Display is a proposed small screen for workplace instructions and status. Your phone creates the update, Bluetooth sends it, and the display keeps it in view.</p><div className="ad-actions"><DisplaySectionLink className="ad-button ad-button--primary" section="pilot">Apply for a pilot</DisplaySectionLink><DisplaySectionLink className="ad-button ad-button--secondary" section="works">See how it works</DisplaySectionLink></div><p className="ad-hero__secondary">Exploring the project? <Link to="/display/invest">Read the investor brief</Link>.</p></div><div className="ad-hero__product"><DisplayAssetFigure asset={displayAssets.heroConcept} className="ad-hero__asset" priority /><div className="ad-screen-tabs" aria-label="Example display states"><span>Room status</span><span>Next task</span><span>Operational alert</span></div></div></div></header>
